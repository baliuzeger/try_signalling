...
impl Agent for Model {
    fn running_connections(&self) -> Vec<RunningPassiveConnection> {
        self.out_connections_2.iter().map(|cn| RunningPassiveConnection::new(cn.connection.upgrade().unwrap())).collect()
        // self.out_connections_1.iter().map(|cn| RunningPassiveConnection::new(Arc::clone(&cn)))
        //     .chain(self.out_connections_2.iter().map(|cn| RunningPassiveConnection::new(Arc::clone(&cn))))
        //     .collect()
    }
